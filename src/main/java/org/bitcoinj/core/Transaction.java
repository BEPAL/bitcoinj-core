...
public class Transaction extends ChildMessage {
    ...
    public Sha256Hash hashForSignature(int inputIndex, byte[] connectedScript, byte sigHashType) {
        ...
        try {
            ...
            //bcd
//            ByteArrayOutputStream bos1 = new UnsafeByteArrayOutputStream(tx.length == UNKNOWN_LENGTH ? 256 : tx.length);
//            tx.bitcoinSerialize(bos1);
//            byte[] arr = bos1.toByteArray();
//
//            ByteArrayOutputStream bos = new UnsafeByteArrayOutputStream(tx.length == UNKNOWN_LENGTH ? 256 : tx.length + 4 + 32);
//            bos.write(arr, 0, 4);
//            byte[] data1 = Hex.fromHexString("135b17701041a20f264a746197bee239ec2fb8ad91265a2045b1882c787aff9a");
//            bos.write(data1, 0, data1.length);
//            bos.write(arr, 4, arr.length - 4);
//
//
...
//            ByteArrayOutputStream bos = new UnsafeByteArrayOutputStream(tx.length == UNKNOWN_LENGTH ? 256 : tx.length + 4);
//            bos.write(arr, 0, 4);
//            Utils.uint32ToByteStreamLE(1513824249, bos);
//            bos.write(arr, 4, arr.length - 4);
//
//            // We also have to write a hash type (sigHashType is actually an unsigned char)
//            uint32ToByteStreamLE(0x000000ff & sigHashType, bos);
...
//            uint32ToByteStreamLE(1 | 0x40 | (79 << 8), bos);  不是BCH
        } catch (IOException e) {
            throw new RuntimeException(e);  // Cannot happen.
        }
