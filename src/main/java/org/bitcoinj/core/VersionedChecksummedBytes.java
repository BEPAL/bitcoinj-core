...
public class VersionedChecksummedBytes implements Serializable, Cloneable, Comparable<VersionedChecksummedBytes> {
    ...
    protected VersionedChecksummedBytes(String encoded) throws AddressFormatException {
        ...
        if (versionAndDataBytes.length == 21) {
            version = versionAndDataBytes[0] & 0xFF;
            bytes = new byte[20];
        } else if (versionAndDataBytes.length == 22) {
            int value1 = versionAndDataBytes[0];
            int value2 = versionAndDataBytes[1];
            version = (value1 & 0xFF) << 8 | value2 & 0xFF;
            bytes = new byte[20];
        } else {
            version = versionAndDataBytes[0] & 0xFF;
            bytes = new byte[versionAndDataBytes.length - 1];
        }
        System.arraycopy(versionAndDataBytes, versionAndDataBytes.length - bytes.length, bytes, 0, bytes.length);
    }
...
//        checkArgument(version >= 0 && version < 256);
    ...
    public final String toBase58() {
        ...
        byte[] addressBytes = new byte[bversion.length + bytes.length + 4];
        System.arraycopy(bversion, 0, addressBytes, 0, bversion.length);
        System.arraycopy(bytes, 0, addressBytes, bversion.length, bytes.length);
        byte[] checksum = Sha256Hash.hashTwice(addressBytes, 0, bytes.length + bversion.length);
        System.arraycopy(checksum, 0, addressBytes, bytes.length + bversion.length, 4);
